...
public class IncisionPicture {
    ...
    public boolean incisionPicture(String picturePath, String saveFolderPath) {
        ...
        try {
            ...
            Log4jLog.log.info("矫正 生成新图片 IOException：" + picturePath);
            return false;
        }
        ...
        for (int x = 0; x < 2; x++) {
            ...
            for (int j = bufferedImage.getMinY() + 30;  // 开始点，减去部分 为旋转部分
                ...
                count = 0;
                for (int i = startMinX + 10; i < startMinX + 50; i++) {  // 判断连续值
                    if (picture.isRgbEquals(i, j) && count++ == PictureNumber.PICTURE_COUNT) {
                        listY.add(j);
